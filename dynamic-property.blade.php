...
.main-gallery-image {
    ...
    position: relative;
    cursor: pointer;
    height: 430px; /* Set a fixed height for the main image */
}
...
.secondary-gallery-image {
    position: relative;
    cursor: pointer;
    height: 212px; /* Approximately half the height of the main image */
}
...
<div class="gallery-section mx-3">
    ...
    <div class="container py-4">
        ...
        <div class="adr-area" style="margin-bottom: 20px;">
            @if($data->address)
                <h6 class="text-secondary-dark"><i class="fa-solid fa-location-dot text-secondary-dark"></i> {{$data->address}}, {{$data->city}}, {{$data->country}}</h6>
            @endif
        </div>
        <div class="position-relative mb-4">
            <div class="gallery-grid" id="imageGrid">
                ...
                @foreach($images as $key => $c)
                    @if($key < 2)
                        <div class="secondary-gallery-image" onclick="openLightbox({{$i}})">
                            <img src="{{($c->url)}}" alt="{{$c->text}}" class="gallery-img {{$key == 1 ? 'top-right-rounded' : ''}}">
                        </div>
                    @endif
                    @php $i++; @endphp
                @endforeach
                ...
                @foreach($images as $key => $c)
                    @if($key >= 2)
                        <div class="secondary-gallery-image" onclick="openLightbox({{$i-2}})">
                            <img src="{{($c->url)}}" alt="{{$c->text}}" class="gallery-img {{$key == 3 ? 'bottom-right-rounded' : ''}}">
                        </div>
                    @endif
                    @php $i++; @endphp
                @endforeach
    ...
    <div id="showAllModal" class="show-all-modal">
        <div class="min-vh-100 p-4">
            <div class="d-flex align-items-center justify-content-between mb-4">
                <h2 class="text-white fs-3 fw-semibold">All Photos - {{$data->name}}</h2>
